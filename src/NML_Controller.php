...
class NML_Controller
{
    ...
    function single(Request $request)
    {
        ...
        try {
            $file = (config('media-library.disk') == 's3') ? $item->image['name'] : $this->folder . '/' . $item->image['name'];
            $size = ((config('media-library.disk') == 's3') ? Storage::disk('s3')->size($file) : File::size($file)) / 1024;
            $type = (config('media-library.disk') == 's3') ? Storage::disk('s3')->mimeType($file) : File::mimeType($file);
        } catch (\Exception $e) {
            return [];
        }
    ...
    function upload(Request $request, NML_Model $model)
    {
        ...
        if (config('media-library.disk') == 's3') {
            $move = $file->storeAs(config('media-library.s3folder'), $file->getClientOriginalName(), 's3');

            if ($move) {
                $model->insert(['title' => $title, 'image' => $move]);
                return ['status' => true];
            }
        } else {
    ...
    function delete(Request $request, NML_Model $model)
    {
        ...
        if (count($get) > 0) {
            foreach ($get as $key) {
                if (config('media-library.disk') == 's3') {
                    Storage::disk('s3')->delete($key->image['name']);
                } else {
